                        ...
                        <form onSubmit={submit}>
                            <Flex direction={'column'} gap={'4'}>
                                <Flex direction={'column'} gap={'2'}>
                                    ...
                                    <Input type={'email'} id={'email'} name={'email'} value={data.email} onChange={(e) => setData('email', e.target.value)} className={`${errors.email ? 'border-red-500' : ''}`} />
                                    <Text className={'text-red-500 text-sm'}>{errors.email}</Text>
                                </Flex>
                                <Flex direction={'column'} gap={'2'}>
                                    <Flex direction={'row'} justify={'between'} align={'center'} gap={'2'}>
                                        <Label htmlFor={'password'}>Mot de passe</Label>
                                        <Link href={route('password.request')} className={'text-sm hover:underline'}>Mot de passe oublié ?</Link>
                                    </Flex>
                                    <Input type={'password'} id={'password'} name={'password'} value={data.password} onChange={(e) => setData('password', e.target.value)} className={`${errors.password ? 'border-red-500' : ''}`} />
                                    <Text className={'text-red-500 text-sm'}>{errors.password}</Text>
                                </Flex>
                                <Flex direction={'row'} gap={'2'}>
                                    <Checkbox id={'remember'} name={'remember'} onChange={(e) => setData('remember', (e.target as HTMLInputElement).checked)} />
                                    <Label htmlFor={'remember'}>Se souvenir de moi</Label>
                                </Flex>
                                <Flex direction={'row'} justify={'end'} align={'center'} gap={'2'}>
                                    <Button type={'submit'} disabled={processing} className={`${processing ? 'cursor-not-allowed' : 'cursor-pointer'} w-full`}>
                                        Connexion {' '} <ArrowRight size={16} />
                                    </Button>
                                </Flex>
                                <Flex direction={'row'} justify={'center'} align={'center'} gap={'2'}>
                                    <Text className={'text-center text-sm'}>Vous n'avez pas de compte ? <Link href={route('register')} className={'hover:underline'}>Inscrivez-vous</Link></Text>
                                </Flex>
                            </Flex>
                        </form>
                    </CardContent>
